...
import { Table, SearchResult } from './product-listing.model';
...
function matches(tables: Table, term: string, pipe: PipeTransform) {
    return tables.ean.toLowerCase().includes(term.toLocaleLowerCase())
        || tables.productName.toLowerCase().includes(term.toLocaleLowerCase())
        || tables.rrp.toString().toLowerCase().includes(term)
        || tables.max_price.toString().toLowerCase().includes(term)
        || tables.min_price.toString().toLowerCase().includes(term);
}
...
export class AdvancedService {
    ...
    set tableData(tableData: Table[]) { this._set({ tableData }); }
    // tslint:disable-next-line: adjacent-overload-signatures
