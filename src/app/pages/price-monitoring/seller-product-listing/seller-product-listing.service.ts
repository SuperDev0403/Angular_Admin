...
import { Table, SearchResult } from './seller-product-listing.model';
...
function matches(tables: Table, term: string, pipe: PipeTransform) {
    return tables.ean.toLowerCase().includes(term)
        || tables.productName.toLowerCase().includes(term.toLowerCase())
        || tables.rrp.toString().includes(term)
        || tables.price.toString().includes(term)
        || tables.diff.toString().includes(term)
        || tables.perdiff.toString().includes(term);
}
