...
import { Table, SearchResult } from './seller-pricing.model';
...
function matches(tables: Table, term: string, pipe: PipeTransform) {
    return tables.seller_name.toLowerCase().includes(term.toLocaleLowerCase())
        || tables.totalProducts.toString().includes(term)
        || tables.bellowCount.toString().includes(term)
        || tables.bellow_perc.toString().includes(term);
}
